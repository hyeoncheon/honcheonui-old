...
class Daemon:
	...
	def __init__(self, name = 'daemon', pidfile = None):
		"""
		orion.runner.Daemon is initilized with name and pidfile.
		pidfile can be omitted and '/tmp/NAME.pid' is used for that case.
		"""
		self.name = name
		...
		self.start_ts = time.time()		# to calculate running time.

		self.__parse_options__()

		# After parsing arguments, all options are stored in self.opts.
		# then, daemon do maintenence operations first.
		if self.opts.stop:
			sys.stderr.write('stopping existing process...\n')
			_process(self.__read_pidlock()).kill_existing()
			sys.exit()

		# check existing agent, then abort or force start after kill old one.
	...
	def __read_pidlock(self):
		try:
			...
			if self.opts.start:
				sys.stderr.write('**** In start mode, it is normal. ignore.\n')
			pid = -e.errno
		return int(pid)
...
Examples:
  ...
  to start daemon:
	--start -c config.xml
	--start -c config.xml -l daemon.log
	--start -c config.xml -l daemon.log -o stdout.log -e stderr.log

  to stop daemon:
	--stop

  to start daemon in debugging mode:
	--start -c config.xml -l daemon.log -d
				'''
				)
	...
	def __print_options__(self):
		...
		for k in vars(self.opts):
			if type(vars(self.opts)[k]).__name__ == 'TextIOWrapper':
				info_text = "- option %s is '%s'" % (k, vars(self.opts)[k].name)
			else:
				info_text = "- option %s is %s" % (k, vars(self.opts)[k])
...
class _process:
	...
	def kill_existing(self):
		...
		if ret == 3:	#  No such process. nothing to do.
		...
		#  Otherwise, wait for graceful exit then do double kill!
